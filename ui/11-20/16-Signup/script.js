...
1. 인풋에 텍스트를 입력 시, input에 filled class가 추가되면서
   라벨 텍스트가 transform: scale(.8) translateY(-80%);이 적용되어 위로 up 되면서
   초록라인이 transition: .3s ease-in-out; 가 적용되어 애니메이션이 좌측에서 우측으로 생김
2. 인풋에 텍스트를 지울 시, 라벨 텍스트가 원래 위치로 down 되면서 초록라인 사라짐
3. signup 버튼을 누를 시 인풋 필드가 하나라도 비어있으면 error class가 생기면서 레드 라인이 생김
...
바로 마이페이지로 이동하거나 메인페이지로 이동하는게 회원가입 step을 줄일수 있음
